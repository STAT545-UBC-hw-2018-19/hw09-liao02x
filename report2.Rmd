...
```{r include = FALSE}
letter_freq <- read.delim("data/letter_freq.tsv")
letter_head <- read.delim("data/letter_head.tsv")
letter_tail <- read.delim("data/letter_tail.tsv")
```

In this report, I explore the `words.txt` further to see if there's more interesting things. Again, it contains `r sum(letter_head$Freq)` words.

## Letter frequency

First I split the words into letters and calculate the frequency at a letter level.

The most frequent letter is `r with(letter_freq, freq[which.max(Freq)])`. I also noticed that there's dash `"-"` in the words. 
...
The most frequent head letter is `r with(letter_head, head[which.max(Freq)])`.
...
The most frequent tail letter is `r with(letter_tail, tail[which.max(Freq)])`.
