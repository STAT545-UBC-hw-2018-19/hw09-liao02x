...
The structure is shown below:
  * (R)[https://github.com/STAT545-UBC-students/hw09-liao02x/tree/master/R]
    - (histogram.R)[]: origin file
    - (letter_freq.R)[]: R script for calculating letter frequency
    - (letter_head.R)[]: R script for calculating head letter frequency
    - (letter_tail.R)[]: R script for calculating tail letter frequency
    - (visualize_makefile.R)[]: R script for visualize Makefile somehow with R
  * (python)[https://github.com/STAT545-UBC-students/hw09-liao02x/tree/master/python]
    - (makefile2dot.py)[]: modified from (here)[] to assign different color for different file type
  * (report.Rmd)[]: origin file
  * (report2.Rmd)[]: additional report for further analysis on the `words.txt`
  * (README.Rmd)[]: building `README.md` file for this repo
